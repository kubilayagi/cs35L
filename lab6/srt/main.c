...
/* nthreads is the number of threads we will use */
int nthreads;

/* declaration of scene as a global  */
scene_t scene;

/* colors array  
we choose width, then height bc the px loop is on the outside
and we use three at the end becuase the output is three colors
*/
float colors[width][height][3];

//this is the new function we make with the code from main
void* get_colors(void* arg) {
...
main( int argc, char **argv )
{
    ...
    */
    ...
    for (i=0; i<nthreads; i++) {
      threadnum[i] = i;
      int ret = pthread_create(&tid[i], NULL, get_colors, (void*)&threadnum[i]);
      if (ret) {
    ...
    for (j=0; j<nthreads; j++) {
      int ret = pthread_join(tid[j], NULL);
      if (ret) {
	...
	printf( "%.0f %.0f %.0f\n",                                                                                           
		colors[x][y][0], colors[x][y][1], colors[x][y][2] );
      }
      printf("\n");
    }
